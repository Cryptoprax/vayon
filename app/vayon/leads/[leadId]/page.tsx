...
export default async function Page({ params, searchParams }: { params: Promise<{ leadId: string }>; searchParams: Promise<{ tab?: string; success?: string }> }) {
  ...
  return <>{query.success && <p role="status" className="mx-auto mt-5 max-w-[96rem] rounded-xl bg-vds-success-soft p-3 text-sm text-vds-success">{query.success}</p>}{query.success && <ContextualAIActions kind="lead" evidence={{ phone: lead.phone, email: lead.email, status: lead.status }} recordId={id} />}<WorkspaceRenderer model={leadWorkspaceModel(lead)} activeTab={active} editHref={`/vayon/leads/${id}/edit`} /><RealEstateSignalGrid kind="lead" /><ContextualRealEstateRecommendations kind="lead" />{!query.success && <ContextualAIActions kind="lead" evidence={{ phone: lead.phone, email: lead.email, status: lead.status }} recordId={id} />}<div className="mx-auto mb-8 max-w-[96rem] px-4 sm:px-6"><EntityCollaboration entityType="lead" entityId={id} entityLabel="Lead" /></div></>;
}
