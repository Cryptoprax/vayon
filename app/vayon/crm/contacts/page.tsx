...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <CrmShell title="Clients" description="Find your clients, their company, and the person managing each relationship."><form className="mb-5"><input name="search" defaultValue={search} placeholder="Search clients" aria-label="Search contacts" className="vds-focus h-11 w-full max-w-md rounded-xl border border-vds-border bg-vds-surface px-3 text-sm"/></form><ContactDirectory items={items}/></CrmShell>;
}
