...
24. **Manual steps and practical limits.** The Sprint 237 SQL and Vercel variables are already configured; do not reapply the migration or recreate those variables for this scheduler fix. Vercel remains Hobby ($0); Vercel Cron is intentionally not used. `.github/workflows/founding-reconciliation.yml` schedules an authenticated GET to `https://www.vayon.online/api/billing/paddle/founding/reconcile` at `7,22,37,52 * * * *` (UTC), with optional maintainer `workflow_dispatch`. Paddle remains Sandbox during current testing. After review and deployment, verify the workflow succeeds. Retain transaction/subscription webhook notifications, and verify API permissions for reading/updating transactions and subscriptions. Confirm in Sandbox that the new price is active, USD $79, monthly, on the existing Professional product, with no trial, and that standard Professional remains $149 monthly/$1,430.40 annual. Exercise payment, abandonment cancellation, pause/resume, and a twelfth-payment transition against Paddle before launch. Real Paddle payment/API mutations were not executed in this session.

    **Scheduler setup.** In `Cryptoprax/vayon`, open Settings > Secrets and variables > Actions > New repository secret. Name it `VAYON_RECONCILIATION_SECRET`; enter the same value as Vercel Production `CRON_SECRET` from your existing secure record. Never commit either value. Do not retrieve or print the Vercel secret. If the original value is unavailable, stop: a separately approved coordinated rotation is required. GitHub receives only this endpoint credential, never Paddle or Supabase credentials. Create the secret when ready to enable scheduled production calls; once this workflow is on default branch `main` and the secret exists, the next schedule can invoke reconciliation automatically. No push trigger or deployment step is included. Manual dispatch also calls production and should only be used after deployment is confirmed.

    **Canonical routing.** `www.vayon.online` is the canonical scheduler target. `vayon.online` redirects to `www` with HTTP 308 and must not be used by the scheduler; redirect following intentionally remains disabled. Both existing secrets (`CRON_SECRET` and `VAYON_RECONCILIATION_SECRET`) remain unchanged; no recreation or rotation is required. HTTP failures report only a validated three-digit status, never response bodies or credentials. Vercel remains Hobby and GitHub Actions remains the 15-minute scheduler.

    **Scheduler safeguards and limits.** Missing/incorrect bearer authorization still returns 401 using the existing timing-safe comparison. The route remains GET, as required by its existing interface. One workflow run at a time is allowed, without canceling a running reconciliation. HTTPS requests have bounded timeouts, do not follow redirects, and do not retry immediately. Non-2xx responses, network failures, invalid JSON, and missing/false `ok` fail the job without printing response bodies. Existing database locks, unique payment records, mutation leases, and provider-state checks remain authoritative for concurrency with webhooks/customer actions. GitHub scheduling is best-effort, not a 15-minute SLA; monitor failed/missed runs, Actions usage limits, and inactivity disabling for public repositories. See [GitHub scheduled events](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule).
