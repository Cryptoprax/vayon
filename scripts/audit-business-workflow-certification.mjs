...
const queries = ['Property','Lead','Client','Deal','Task','Campaign','Invite Team','Reports','Settings','Create Property','Create Lead','Generate Brochure','Schedule Viewing','Call Buyer'];
...
const workspace = {
  ...
  ownershipOnboardingDependent:read('features/onboarding/services/onboarding.service.ts').includes('complete_sprint43_onboarding'),
...
const report={generatedAt:new Date().toISOString(),status:'Blocked',scope:'Source contracts and local pure-function execution; not production/customer certification',productionVerified:false,firstTimeCustomerObserved:false,roles:roleResults,workspace,routeFiles:readdirSync('app',{recursive:true}).filter(file=>file.endsWith('page.tsx')).length};
mkdirSync('test-results/business-certification',{recursive:true});
writeFileSync('test-results/business-certification/source-audit.json',JSON.stringify(report,null,2));
console.log(JSON.stringify({status:report.status,roles:roles.length,searchCases:queries.length*roles.length,workspace:workspace.status,artifact:'test-results/business-certification/source-audit.json'},null,2));
