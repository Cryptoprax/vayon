...
function ContentCalendar() {
  ...
  const phases = ["Planning", "Review", "Creative", "Approval", "Launch", "Completed"];
  return <section className="rounded-3xl border border-vds-border bg-vds-surface p-5" aria-labelledby="content-calendar-title"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 id="content-calendar-title" className="font-semibold">{view} view</h3><p className="mt-1 text-xs text-vds-muted">Campaigns may span multiple days from planning through completion.</p></div><div role="group" aria-label="Calendar view" className="flex rounded-xl border border-vds-border bg-vds-input p-1">{(["Day", "Week", "Month"] as const).map((item) => <Button key={item} size="sm" variant="control" aria-pressed={view === item} onClick={() => setView(item)} className={`rounded-lg px-3 ${view === item ? "bg-vds-surface text-vds-primary" : "text-vds-muted"}`}>{item}</Button>)}</div></div><div className="mt-5 overflow-x-auto"><ol className="flex min-w-[42rem] gap-2" aria-label="Campaign planning phases">{phases.map((phase) => <li key={phase} className="min-w-28 flex-1 rounded-2xl border border-vds-border bg-vds-input p-3 text-center text-xs font-medium">{phase}</li>)}</ol></div><div className="mt-4 grid min-h-40 place-items-center rounded-2xl border border-dashed border-vds-border p-6 text-center"><div><CalendarDays className="mx-auto size-8 text-vds-primary" aria-hidden="true" /><p className="mt-3 font-medium">Prepare the first multi-day campaign for your {view.toLowerCase()} view.</p><p className="mt-1 text-sm text-vds-muted">Include platform, objective, audience, campaign, owner, approval, and publishing date.</p></div></div></section>;
}
