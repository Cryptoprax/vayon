...
export function CommercialPlans({ catalog, workspaceId, clientToken, environment = "live", subscribed = false, onCheckout, onMessage }: {
  ...
  return <section className="mt-5" aria-label="Compare subscription plans">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{(["starter", "professional", "business", "business_plus"] as const).map(code => {
      ...
      return <article key={code} className={card}><h3 className="text-lg font-semibold">{plan.name}</h3><p className="mt-2 text-sm text-vds-muted">{plan.audience}</p><p className="mt-4 text-2xl font-semibold">{formatted ?? "Pricing is being updated"}</p>{price && <p className="text-sm text-vds-muted">per {period === "monthly" ? "month" : "year"}</p>}
        ...
        {price && clientToken && !subscribed ? <Button variant="primary" disabled={busy !== null} onClick={() => checkout(code)}>{busy === code ? "Opening checkout…" : "Upgrade to " + plan.name}</Button> : <p className="text-sm text-vds-muted">{subscribed ? "Manage your current subscription below." : "Online checkout is temporarily unavailable."}</p>}
      </article>;
