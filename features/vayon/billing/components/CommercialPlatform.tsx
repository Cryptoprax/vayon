...
import { refreshSubscriptionState, refreshFoundingAvailability } from "../actions/subscription-center.actions";

const card = "min-w-0 rounded-2xl border border-vds-border bg-vds-surface p-5";
type Period = "monthly" | "annual";

export function CommercialPlans({ workspaceId, clientToken, environment = "live", subscribed = false, checkoutEnabled = false, founding = unavailableFoundingOffer, onCheckout, onMessage }: {
  catalog: PublicPaddleCatalogPrice[]; organizationId: string; workspaceId: string; clientToken?: string; environment?: "sandbox" | "live"; subscribed?: boolean; checkoutEnabled?: boolean; founding?: FoundingMemberAvailability; onCheckout?: () => void; onMessage?: (message: string) => void;
}) {
  ...
  return <section className="mt-5" aria-label="Compare subscription plans">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{selfServiceCommercialPlans.map(displayPlan => {
      ...
      return <article key={displayPlan.code} className={`${card} flex h-full flex-col`}>
        ...
        {period === "annual" && <p className="mt-3 text-xs text-vds-muted">${((displayPlan.standardMonthlyPrice ?? 0) * 12 * 0.8).toFixed(2)} billed annually. Annual pricing uses the standard plan rate.</p>}
