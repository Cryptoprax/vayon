...
export async function synchronizeGmailAction(){try{const result=await new GmailProductionSyncService().synchronize();revalidatePath("/vayon/email");revalidatePath("/vayon/home");back("success",`Gmail synchronized: ${result.imported} new, ${result.duplicates} already current, ${result.linked} linked to CRM.`)}catch(reason){back("error",reason instanceof Error?reason.message:"Gmail synchronization failed. Try again or reconnect Gmail.")}}
