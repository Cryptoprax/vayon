...
export class IntegrationCenterService {
  async model(): Promise<IntegrationCenterModel> {
    ...
    const [google, microsoft, whatsapp, calendarConnection, openAIConnection, healthResult, ...featureFlags] = await Promise.all([
      ...
      ctx.client.from("integration_connections").select("configuration,integration_providers!inner(code)").eq("workspace_id",ctx.workspaceId).eq("integration_providers.code","openai").is("deleted_at",null).maybeSingle(),
    ...
    const items = integrationCenterRegistry.map((definition): IntegrationCenterItem => {
      ...
      const paddleLive = process.env.PADDLE_ENVIRONMENT === "live" && Boolean(process.env.PADDLE_API_KEY) && !process.env.PADDLE_API_KEY?.includes("_sdbx_") && Boolean(process.env.PADDLE_WEBHOOK_SECRET);
      const configured = definition.code === "openai" ? Boolean(process.env.OPENAI_API_KEY) : definition.code === "paddle" ? paddleLive : definition.code === "transactional_email" ? Boolean(process.env.EMAIL_PROVIDER && process.env.EMAIL_FROM_ADDRESS) : false;
      ...
      return {
        ...
        operational: {
          ...
          configuredModel: definition.code === "openai" ? String((openAIConnection.data?.configuration as {model?:string}|null)?.model ?? process.env.OPENAI_MODEL ?? "gpt-5.5") : null,
          ...
          environmentMode: definition.code === "paddle" ? (process.env.PADDLE_ENVIRONMENT === "live" ? "Live" : "Sandbox") : null,
          ...
          portalHealth: definition.code === "paddle" ? (paddleLive ? "Connected" : "Not Connected") : null,
