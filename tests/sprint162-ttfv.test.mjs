...
test("supported businesses receive short first-value journeys and one primary objective", () => {
  for (const business of [
    ...
    assert.ok(adaptive.includes(business));
  for (const match of adaptive.matchAll(/journey:\s*\[(.*?)\], modules:/gs))
    assert.ok((match[1].match(/id:/g) ?? []).length <= 5);
...
test("progress, samples, one contextual hint, and success moments are present", () => {
  ...
  assert.match(adaptive, /Explore Demo Workspace/);
  assert.match(adaptive, /Use Sample Data/);
  assert.equal(
    (adaptive.match(/aria-label=\"Contextual hint\"/g) ?? []).length,
    1,
  );
...
test("priority empty states recommend an action and route", () => {
  for (const file of [
    ...
    assert.doesNotMatch(
      source,
      /No (?:properties|records|conversations|timeline items|campaign data|governed notifications) (?:are|is) available/,
    );
