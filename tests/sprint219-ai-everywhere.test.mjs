...
test("primary entities expose one shared contextual AI action surface",()=>{for(const kind of ["property","lead","client","company","dashboard"])assert.match(actions,new RegExp(`${kind}:`));for(const page of ["app/vayon/properties/[propertyId]/page.tsx","app/vayon/leads/[leadId]/page.tsx","app/vayon/crm/contacts/[contactId]/page.tsx","app/vayon/crm/companies/[companyId]/page.tsx","app/vayon/deals/[dealId]/page.tsx"])assert.match(read(page),/ContextualAIActions/)});
...
test("approval status memory and automation guidance are visible without duplicate chat",()=>{for(const value of ["No prepared work is confirmed","Requires approval","More actions for this"])assert.match(actions,new RegExp(value));assert.doesNotMatch(actions,/textarea|role="dialog"|position:fixed/)});
