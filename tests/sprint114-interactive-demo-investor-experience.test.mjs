...
test("demo remains isolated read-only seeded fixture architecture", async () => {
  const [contracts, component, route] = await Promise.all([
    read("features/vayon/demo-experience/domain/contracts.ts"),
  ...
  assert.match(component, /data-demo-tenant="prime-properties-demo-workspace"/);
...
test("all five audience modes are explicit and selectable", async () => {
  ...
  for (const mode of ["visitor", "sales", "investor", "founder", "enterprise"])
